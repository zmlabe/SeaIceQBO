...
directorydata3 = '/surtsey/zlabe/simu/CTLQ/monthly/'
...
### CTLQ -- Call function for surface temperature data from reach run
data = Dataset(directorydata3 + '%s_1801-2000.nc' % varnames[v])
uctlqq = data.variables[varnames[v]][:]
data.close()

uctlqq = np.asarray(uctlqq)
uctlqq[np.where(uctlqq < -999)] = np.nan

### Create 2d array of latitude and longitude
latq = np.where((lat>=-5) & (lat<=5))[0]
uctlq = np.nanmean(uctlqq[:,:,latq,:],axis=2)
uctlqzonal = np.nanmean(uctlq[:,:,:],axis=2)

### Calculate anomalies
meanuctlq = np.nanmean(uctlqzonal,axis=0)
anomctlq = uctlqzonal - meanuctlq

ustackctlq = anomctlq

lon2,lat2 = np.meshgrid(lon,lat[latq])

ustackctlq[np.where(ustackctlq<-40)] = np.nan
...
cs = plt.contourf(timeq[:,:501],levq[:,:501],ustackctlq[:501].transpose(),limit,extend='both')
...
plt.savefig(directoryfigure + 'QBO_CTLQ.png',dpi=300)
