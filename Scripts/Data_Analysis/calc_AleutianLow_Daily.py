...
def readVariables(varnames,period,location):
    ...
    if varnames == 'SLP': # North Pacific Index (Trenberth and Hurrell, 1994)
...
slp,lat,lon,lev = readVariables('SLP',period,'NorthPacific')
...
plt.yticks(np.arange(0,0.25,0.02),list(map(str,np.arange(0,0.25,0.02))),
           fontsize=10)
plt.xticks(np.arange(990,1031,5),list(map(str,np.arange(990,1031,5))),
           fontsize=10) 
plt.xlim([990,1030])
plt.ylim([0,0.08])
...
plt.savefig(directoryfigure + 'AleutianLow_Daily.png',dpi=300)