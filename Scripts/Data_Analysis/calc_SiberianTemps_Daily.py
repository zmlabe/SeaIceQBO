...
Script calculates Siberian temperature distribution.
...
print('\n' '----Calculating Siberian Temps - %s----' % titletime)
...
#            tas_mo[i] = np.nanmean(runs[i][:,-2:,:,:],axis=1)
    ...
    tas_mohitnon = tas_mo[0][non_hit,:,:,:] - 273.15
    tas_mofictnon = tas_mo[1][non_fict,:,:,:] - 273.15
...
slp,lat,lon,lev = readVariables('T1000',period,'Siberia')
...
plt.savefig(directoryfigure + 'SiberianTemps_Daily.png',dpi=300)