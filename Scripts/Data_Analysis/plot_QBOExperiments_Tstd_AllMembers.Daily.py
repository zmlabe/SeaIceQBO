...
for v in range(len(varnames)):
    ...
    ### Compute standard deviation of temperature at each grid point
    ...
    for ens in range(var_wfitpos.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfitpos[ens,i,j] = np.nanstd(var_wfitpos[ens,:,i,j],axis=0)
                ten_whitpos[ens,i,j] = np.nanstd(var_whitpos[ens,:,i,j],axis=0)
                ten_wfictpos[ens,i,j] = np.nanstd(var_wfictpos[ens,:,i,j],axis=0)
    ...
    for ens in range(var_wfitnon.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfitnon[ens,i,j] = np.nanstd(var_wfitnon[ens,:,i,j],axis=0)
                ten_whitnon[ens,i,j] = np.nanstd(var_whitnon[ens,:,i,j],axis=0)
                ten_wfictnon[ens,i,j] = np.nanstd(var_wfictnon[ens,:,i,j],axis=0)
    ...
    for ens in range(var_wfitneg.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfitneg[ens,i,j] = np.nanstd(var_wfitneg[ens,:,i,j],axis=0)
                ten_whitneg[ens,i,j] = np.nanstd(var_whitneg[ens,:,i,j],axis=0)
                ten_wfictneg[ens,i,j] = np.nanstd(var_wfictneg[ens,:,i,j],axis=0)
    ...
    plt.savefig(directoryfigure + '/QBO_std_2/QBOExperiments_%s_%sSTD_All.png' % (
                                                                  monthqq,
                                                                  varnames[v]),
                                                                  dpi=300)
