"""
Calculate linear interference for net FSUB-CIT
...
for v in range(len(varnames)):
    ...
    lat,lon1,time,lev,varcit = MO.readExperiAll('%s' % varnames[v],
                                             'CIT','profile')
    lat,lon1,time,lev,varfsub = MOR.readExperiAllRegional('%s' % varnames[v],
                                             'FSUB','profile')
    ...
    ### Concatonate runs
    runnames = [r'CIT',r'FSUB']
    experiments = [r'\textbf{FSUB--CIT}']
    runs = [varcit,varfsub]
    
    ### Separate per periods (D)
    var_d = [varcit[:,-1,:,:,:],varfsub[:,-1,:,:,:]]
    
    ### Take at 60N 
    latq = np.where((lat>59) & (lat<61))[0]
    
    ### Compute comparisons for M - taken ensemble average at 60N (index 79)
    diff_FSUBCIT = np.nanmean(var_d[1] - var_d[0],axis=0)
    diffruns_d = diff_FSUBCIT[:,latq,:].squeeze()
...
plt.savefig(directoryfigure + 'linearInterference_December_FSUBCIT_All.png',dpi=300)
