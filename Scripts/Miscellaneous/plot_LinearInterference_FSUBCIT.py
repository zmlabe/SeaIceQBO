"""
Calculate linear interference for FSUB-CIT
...
for v in range(len(varnames)):
    ...
    lat,lon1,time,lev,tascit = MO.readExperiAll('%s' % varnames[v],
                                                'CIT','profile')
    lat,lon1,time,lev,tasfsub = MOR.readExperiAllRegional('%s' % varnames[v],
                                                'FSUB','profile')
    ...
    ### Compute comparisons for fsubcit - taken ensemble average at 60N (index 79)
    ### +QBO
    diff_FSUBCITpos = np.nanmean(tas_mofsubpos - tas_mocitpos,axis=0)
    diffrunspos = diff_FSUBCITpos[:,latq,:].squeeze() 
    ...
    ### Neutral QBO
    diff_FSUBCITnon = np.nanmean(tas_mofsubnon - tas_mocitnon,axis=0)
    diffrunsnon = diff_FSUBCITnon[:,latq,:].squeeze() 
    ...
    ### -QBO
    diff_FSUBCITneg = np.nanmean(tas_mofsubneg - tas_mocitneg,axis=0)
    diffrunsneg = diff_FSUBCITneg[:,latq,:].squeeze() 
