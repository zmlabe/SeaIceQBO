...
for v in range(len(varnames)):
    ...
    for ens in range(var_wfsubpos.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfsubpos[ens,i,j] = np.nanpercentile(var_wfsubpos[ens,:,i,j],10)
                ten_wcitpos[ens,i,j] = np.nanpercentile(var_wcitpos[ens,:,i,j],10)
                ten_wfpolpos[ens,i,j] = np.nanpercentile(var_wfpolpos[ens,:,i,j],10)
    ...
    for ens in range(var_wfsubnon.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfsubnon[ens,i,j] = np.nanpercentile(var_wfsubnon[ens,:,i,j],10)
                ten_wcitnon[ens,i,j] = np.nanpercentile(var_wcitnon[ens,:,i,j],10)
                ten_wfpolnon[ens,i,j] = np.nanpercentile(var_wfpolnon[ens,:,i,j],10)
    ...
    for ens in range(var_wfsubneg.shape[0]):
        for i in range(lat.shape[0]):
            for j in range(lon.shape[0]):
                ten_wfsubneg[ens,i,j] = np.nanpercentile(var_wfsubneg[ens,:,i,j],10)
                ten_wcitneg[ens,i,j] = np.nanpercentile(var_wcitneg[ens,:,i,j],10)
                ten_wfpolneg[ens,i,j] = np.nanpercentile(var_wfpolneg[ens,:,i,j],10)
...
#                         linewidths=0.4)
    ...
    plt.savefig(directoryfigure + '/QBO_Ext_2/QBOExperiments_%s_%s_Regional.png' % (
                                                                  monthqq,
                                                                  varnames[v]),
                                                                  dpi=300)
