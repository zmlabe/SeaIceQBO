"""
Calculate linear interference for net FPOL-CIT
...
for v in range(len(varnames)):
    ...
    var_d = [varcit[:,-1,:,:,:],varfpol[:,-1,:,:,:]]
    
    ### Take at 60N 
    latq = np.where((lat>59) & (lat<61))[0]
    
    ### Compute comparisons for M - taken ensemble average at 60N (index 79)
    diff_FPOLCIT = np.nanmean(var_d[1] - var_d[0],axis=0)
    diffruns_d = diff_FPOLCIT[:,latq,:].squeeze()
...
plt.savefig(directoryfigure + 'linearInterference_December_FPOLCIT_All.png',dpi=300)
