...
def readVariables(varnames,period,location):
    ...
    if varnames == 'SLP':
        ...
        latq = np.where((lat >= 65) & (lat <= 90))[0]
        lat2s = lat2[latq,:]
        ficthitpos = tas_mofictpos[:,latq,:] 
        ficthitnon = tas_mofictnon[:,latq,:] 
        ficthitneg = tas_mofictneg[:,latq,:] 
        ficthitpos = UT.calc_weightedAve(ficthitpos,lat2s)
        ficthitnon = UT.calc_weightedAve(ficthitnon,lat2s)
        ficthitneg = UT.calc_weightedAve(ficthitneg,lat2s)
    diffruns = [ficthitpos.squeeze(),ficthitnon.squeeze(),ficthitneg.squeeze()]
...
plt.scatter(z30g[0],slp[0],color='deepskyblue',marker='o',label=r'QBO-W')
timex = np.linspace(22000,23201,len(z30g[0]))
slope, intercept, r_value1, p_value1, std_err = sts.linregress(z30g[0],slp[0])
line1 = slope*timex + intercept
plt.plot(timex,line1,color='deepskyblue',linewidth=3)

plt.scatter(z30g[1],slp[1],color='gold',marker='o',label=r'QBO-N')
timex = np.linspace(22000,23201,len(z30g[1]))
slope, intercept, r_value2, p_value2, std_err = sts.linregress(z30g[1],slp[1])
line1 = slope*timex + intercept
plt.plot(timex,line1,color='gold',linewidth=3)

plt.scatter(z30g[2],slp[2],color='crimson',marker='o',label=r'QBO-E')
timex = np.linspace(22000,23201,len(z30g[2]))
slope, intercept, r_value3, p_value3, std_err = sts.linregress(z30g[2],slp[2])
line1 = slope*timex + intercept
plt.plot(timex,line1,color='crimson',linewidth=3)
...
plt.ylabel(r'\textbf{hPa}',fontsize=12)
        
plt.legend(shadow=False,fontsize=12,loc='lower center',
           fancybox=True,frameon=False,ncol=3,bbox_to_anchor=(0.5, -0.15),
           labelspacing=0.2,columnspacing=0.4,handletextpad=0.4)

plt.savefig(directoryfigure + 'PolarVortexSiberianHighRelationship_FICT.png',dpi=300)